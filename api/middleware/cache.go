...
func fetchFromCache(ctx *gin.Context, flags CacheKeyFlags) *CachedResponse {
	...
	str, err := redisCache.Get(context.Background(), key).Result()
	data := []byte(str)
	if err != nil && err != redis.Nil {
		logger.Error("Failed to fetch %s: %v", key, err)
		return nil
	}
	...
	if _, err := response.UnmarshalMsg(data); err != nil {
		logger.Error("Failed to unmarshal %s: %v", key, err)
		return nil
	}
...
func writeCachedResponse(ctx *gin.Context, response *CachedResponse) {
	...
	if err != nil {
		logger.Error("Failed to write cached body: %v", err)
	}
}
...
func storeToCache(ctx *gin.Context, response *CachedResponse,
	...
	redisCache, err := cache.GetRedis()
	if err != nil {
		logger.Error("Failed to get Redis connection: %v", err)
		return
	}
	...
	if err != nil {
		logger.Error("Failed to marshal cached response for %s: %v", key, err)
		return
	}
	// Try to store cached response to Redis.
	_, err = redisCache.Set(context.Background(), key, data, timeout).Result()
	if err != nil {
		logger.Error("Failed to store %s: %v", key, err)
		return
	}
}
