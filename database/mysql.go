...
func (db *mysqlDB) initialize(ctx context.Context, cfg dbConfig) {
	...
	logging.Info("open %s:%s@%s/%s?%s", cfg.Username, cfg.Password,
		host, cfg.DBName, params)

	// Connect to the MySQL database.
	var err error
	db.DB, err = gorm.Open(mysql.Open(dbSource))
	logging.Info("gorm open success")

	if err != nil {
		logging.Error("mysql init err %v", err)
		panic(err)
	}
}
...
func (db *mysqlDB) finalize() {
	...
	// 	logging.Error("Failed to close database handle: %v", err)
	// }
}
