...
func New(tx *gorm.DB, bBook *models.BBookModel) {
...
func GetFirstRecord(tx *gorm.DB) *models.BBookModel {
	result := &models.BBookModel{}
	err := tx.Table(table).
...
func Gets(tx *gorm.DB, query *QueryModel) []*models.BBookModel {
	var result []*models.BBookModel
	err := tx.Table(table).
		Select(
			"id AS id," +
	...
	if gorm.IsRecordNotFoundError(err) {
		return make([]*models.BBookModel, 0)
	}
...
func Get(tx *gorm.DB, query *QueryModel) *models.BBookModel {
	result := &models.BBookModel{}
	err := tx.Table(table).
		Select(
			"id AS id," +
...
func SumGroupByProductCode(tx *gorm.DB, query *QueryModel) []*models.BBookModel {
	var result []*models.BBookModel
	err := tx.Table(table).
		Select(
...
func Modify(tx *gorm.DB, bBook *models.BBookModel, attrs map[string]interface{}) {

	err := tx.Table(table).
		Model(models.BBookModel{}).
		Where("id = ?", bBook.ID).
		Updates(attrs).Error
